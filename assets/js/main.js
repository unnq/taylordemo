// Taylor Webvitation — light interactions only
(function () {
const nav = document.getElementById('nav');
const vellums = document.querySelectorAll('.vellum');
...
// Subtle vellum parallax
const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
if (!prefersReduced) {
const y = Math.min(12, window.scrollY * 0.03);
vellums.forEach(v => (v.style.transform = `translateY(${y}px)`));
}
};
